...
export const BlockBlastTray: FC<BlockBlastTrayProps> = memo(({ tray, trayCellSize, draggingIndex, onPointerDown }) => {
  ...
  return (
    <div class='bb-tray' role='toolbar' aria-label='available pieces' aria-describedby='bb-keyboard-hint'>
      {tray.map((piece, i) => (
        ...
        >
          {piece !== null && <BlockBlastPiece piece={piece} cellSize={trayCellSize} dim={draggingIndex === i} />}
        </div>
      ))}
    </div>
  )
